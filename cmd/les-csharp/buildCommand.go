...
import (
	...
	csharp "github.com/nicdex/les-csharp/pkg/eml/generate"
)

const csharpTemplateDownloadUrl = "https://esp-templates.azurewebsites.net/esp-csharp-template-latest.tar.gz"
...
func ensureTemplateExists(templateDir string, lang string) {
	...
	var templateURL = csharpTemplateDownloadUrl
...
func buildAPIFrom(inputFile string) error {
	...
	_ = os.MkdirAll(buildDir, os.ModePerm)
	ensureTemplateExists(templateDir, "csharp")
	csharp.Generate(markup, buildDir, templateDir)
	...
	fmt.Printf("Start API:\tcd api && xbuild && docker-compose up -d\n")
	return nil
}
